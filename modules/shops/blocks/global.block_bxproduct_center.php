...
if (!nv_function_exists('nv_global_bxproduct_center')) {
    ...
    {
        global $site_mods, $global_config, $module_config, $lang_module, $module_name, $global_array_shops_cat, $db_config, $my_head, $db, $pro_config, $money_config, $blockID, $nv_Cache,$global_array_group;
        ...
        $db->sqlreset()
            ->select('t1.id, t1.listcatid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias,t1.' . NV_LANG_DATA . '_hometext AS hometext, t1.homeimgfile, t1.homeimgthumb , t1.homeimgalt, t1.showprice,t1.product_price as product_price,t1.product_number as product_number')
        ...
        foreach ($list as $row) {
            ...
            $xtpl->assign('PID', $row['id']);
            $xtpl->assign('TITLE', $row['title']);
            $xtpl->assign('HOMETEXT', nv_clean60($row['hometext'], 70));
            ...
            if ($pro_config['active_order'] == '1' and $pro_config['active_order_non_detail'] == '1') {
                if ($row['showprice'] == '1' && !empty($row['product_price'])) {
                    if ($row['product_number'] > 0 or !empty($pro_config['active_order_number'])) {
