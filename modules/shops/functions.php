...
function GetDataIn($result, $catid)
{
    ...
    while (list (
        ...
        if ($homeimgthumb == 1) {
            $thumb = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $homeimgfile;//Tịnh sửa NV_FILES_DIR thành NV_UPLOADS_DIR
...
function GetDataInGroups($result, $array_g)
{
    ...
    while (list ($id, $listcatid, $publtime, $title, $alias, $hometext, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_number, $product_price, $showprice, $gift_content, $newday) = $result->fetch(3)) {
        if ($homeimgthumb == 1) {
            ...
            $thumb = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $homeimgfile;//Tịnh đổi NV_FILES_DIR thành NV_UPLOADS_DIR
        } elseif ($homeimgthumb == 2) {
            // image file
